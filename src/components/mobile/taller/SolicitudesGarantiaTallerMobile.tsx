...
export const SolicitudesGarantiaTallerMobile: React.FC<Props> = ({
  ...
  const handleOpenWarrantyDetail = (w: WarrantyRequest) => {
    ...
    setDetailFormData({
      ...
      tallerOrigin: w.tallerOrigin || currentWorkshopName,
  ...
  const handleSaveWarrantyDetail = (e?: React.FormEvent) => {
    ...
    const updatedRequest: WarrantyRequest = {
      ...selectedWarrantyForDetail,
      clientName: detailFormData.clientName,
      clientPhone: detailFormData.clientPhone,
      motorcycleBrand: detailFormData.motorcycleBrand,
      motorcycleModel: detailFormData.motorcycleModel,
      motorcyclePlate: detailFormData.motorcyclePlate,
      motorcycleVin: detailFormData.motorcycleVin,
      motorNumber: detailFormData.motorNumber,
      motorcycleMileage: Number(detailFormData.motorcycleMileage) || 0,
      warrantyType: detailFormData.warrantyType,
      issueDescription: detailFormData.issueDescription,
      mechanicDiagnosis: detailFormData.mechanicDiagnosis,
      partsRequired: detailFormData.partsRequired,
      estimatedCost: parseFloat(detailFormData.estimatedCost) || selectedWarrantyForDetail.estimatedCost,
      matrizNotes: detailFormData.matrizNotes,
    };

    // Actualizar localmente
    setSelectedWarrantyForDetail(updatedRequest);
    setLocalWarranties((prev) => {
      const next = prev.map((item) => (item.id === updatedRequest.id ? updatedRequest : item));
  ...
  return (
    <div className="w-full flex flex-col min-h-0 space-y-3">
          ...
          <div className="w-full">
                ...
                <div className="bg-white border border-zinc-200 rounded-xl p-3 shadow-2xs space-y-2.5">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setDetailFormData({ ...detailFormData, clientIdNumber: e.target.value })}
                      className="w-full px-2.5 py-1.5 bg-zinc-50 border border-zinc-300 rounded-lg text-xs font-mono font-bold text-zinc-800 outline-none"
                    />
                  </div>
            ...
            {detailActiveTab === 'diagnostico' && (
              <div className="space-y-3 animate-fade-in pt-1 pb-3">
                ...
                {/* Bloque 4: Estado y Observaciones de Matriz */}
                <div className="bg-white border border-zinc-200 rounded-xl p-3 shadow-2xs space-y-2.5">
                  <div className="flex items-center gap-1.5 pb-1 border-b border-zinc-100 text-xs font-bold text-zinc-800 uppercase tracking-wider">
                    <FileCheck2 className="w-3.5 h-3.5 text-purple-600" />
                    <span>Estado & Dictamen de Matriz</span>
                  </div>

                  <div className="p-2.5 bg-zinc-50 rounded-xl border border-zinc-200 flex items-center justify-between">
                    <div>
                      <span className="text-[10px] text-zinc-500 font-bold uppercase block">Estado Actual:</span>
                      <span className="text-xs font-bold text-zinc-900">
                        {getStatusBadge(detailFormData.status).label}
                      </span>
                    </div>
                    <span
                    ...
                    >
                      {detailFormData.status.toUpperCase()}
                    </span>
                  </div>

                  {detailFormData.matrizNotes ? (
                    <div className="p-2.5 bg-blue-50/60 rounded-xl border border-blue-200 text-xs space-y-1">
                      <span className="text-[10px] font-bold text-blue-900 uppercase block">
                        Observaciones de Matriz Central:
                      </span>
                      <p className="text-blue-800">{detailFormData.matrizNotes}</p>
                    </div>
                  ) : (
                    <p className="text-[11px] text-zinc-400 italic">Sin observaciones de Matriz aún.</p>
                  )}
                </div>
                ...
                <div className="bg-white border border-zinc-200 rounded-xl p-3 shadow-2xs space-y-2">
                  ...
                  {selectedWarrantyForDetail.diagnosticPhotos && selectedWarrantyForDetail.diagnosticPhotos.length > 0 ? (
                    ...
                    <div className="py-4 border-2 border-dashed border-zinc-200 rounded-xl flex flex-col items-center justify-center text-center bg-zinc-50/50">
                      ...
                      <span className="text-[10px] text-zinc-400">Adjunte fotografías de diagnóstico para Matriz</span>
                    </div>
                  )}
                </div>
              </div>
      ...
      {/* ========================================================================= */}
      {/* 2. MODAL PARA CREAR NUEVA SOLICITUD DE GARANTÍA                            */}
      {/* ========================================================================= */}
      {showCreateModal && !selectedWarrantyForDetail && (
        <div className="bg-white border-2 border-blue-500 rounded-2xl p-4 shadow-xl space-y-3 animate-fade-in -mt-1.5">
          ...
          <form onSubmit={handleCreateNewRequest} className="space-y-2.5">
            <div>
              <label className="block text-[11px] font-bold text-zinc-700 mb-0.5">Nombre del Cliente *</label>
              <input
                type="text"
                ...
                required
                className="w-full px-2.5 py-1.5 text-xs bg-zinc-50 border border-zinc-300 rounded-lg outline-none focus:border-blue-600"
              />
            </div>

            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-[11px] font-bold text-zinc-700 mb-0.5">Cédula</label>
                <input
                  type="text"
                  value={newRequestData.clientIdNumber}
                  onChange={(e) => setNewRequestData({ ...newRequestData, clientIdNumber: e.target.value })}
                  placeholder="0912345678"
                  className="w-full px-2.5 py-1.5 text-xs font-mono bg-zinc-50 border border-zinc-300 rounded-lg outline-none focus:border-blue-600"
                />
              </div>
              <div>
                <label className="block text-[11px] font-bold text-zinc-700 mb-0.5">Teléfono</label>
                <input
                  type="tel"
                  value={newRequestData.clientPhone}
                  onChange={(e) => setNewRequestData({ ...newRequestData, clientPhone: e.target.value })}
                  placeholder="0991234567"
                  className="w-full px-2.5 py-1.5 text-xs font-mono bg-zinc-50 border border-zinc-300 rounded-lg outline-none focus:border-blue-600"
                />
              </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-[11px] font-bold text-zinc-700 mb-0.5">Marca Moto</label>
                <input
                  type="text"
                  value={newRequestData.motorcycleBrand}
                  onChange={(e) => setNewRequestData({ ...newRequestData, motorcycleBrand: e.target.value })}
                  placeholder="Benelli"
                  className="w-full px-2.5 py-1.5 text-xs bg-zinc-50 border border-zinc-300 rounded-lg outline-none focus:border-blue-600"
                />
              </div>
              <div>
                <label className="block text-[11px] font-bold text-zinc-700 mb-0.5">Modelo Moto</label>
                <input
                  type="text"
                  value={newRequestData.motorcycleModel}
                  onChange={(e) => setNewRequestData({ ...newRequestData, motorcycleModel: e.target.value })}
                  placeholder="TRK 502X"
                  className="w-full px-2.5 py-1.5 text-xs bg-zinc-50 border border-zinc-300 rounded-lg outline-none focus:border-blue-600"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <input
                  type="text"
                  value={newRequestData.motorcyclePlate}
                  onChange={(e) => setNewRequestData({ ...newRequestData, motorcyclePlate: e.target.value.toUpperCase() })}
                  placeholder="PBX-1234"
                  className="w-full px-2.5 py-1.5 text-xs font-mono font-bold uppercase bg-zinc-50 border border-zinc-300 rounded-lg outline-none focus:border-blue-600"
                />
              </div>
              <div>
                <label className="block text-[11px] font-bold text-zinc-700 mb-0.5">Tipo Garantía</label>
                <select
                  value={newRequestData.warrantyType}
                  onChange={(e) =>
                    setNewRequestData({ ...newRequestData, warrantyType: e.target.value as 'marca' | 'plus_taller' | 'gps' })
                  }
                  className="w-full px-2 py-1.5 text-xs bg-zinc-50 border border-zinc-300 rounded-lg outline-none focus:border-blue-600"
                >
                  <option value="marca">Oficial Fábrica</option>
                  <option value="plus_taller">Taller Plus</option>
                  <option value="gps">Seguridad GPS</option>
            ...
            <div>
              <label className="block text-[11px] font-bold text-zinc-700 mb-0.5">Descripción del Reclamo / Falla *</label>
              <textarea
                rows={2}
                value={newRequestData.issueDescription}
                onChange={(e) => setNewRequestData({ ...newRequestData, issueDescription: e.target.value })}
                placeholder="Describa el problema presentado por la motocicleta..."
                required
                className="w-full px-2.5 py-1.5 text-xs bg-zinc-50 border border-zinc-300 rounded-lg outline-none focus:border-blue-600 resize-none"
              />
            </div>
            ...
            <div className="flex gap-2 pt-2 border-t border-zinc-100">
              <button
                type="button"
                onClick={() => setShowCreateModal(false)}
                className="flex-1 py-1.5 border border-zinc-300 text-zinc-700 rounded-xl text-xs font-bold hover:bg-zinc-50 cursor-pointer"
              >
                Cancelar
              </button>
              <button
                type="submit"
                className="flex-1 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold shadow-xs cursor-pointer"
              >
                Enviar a Matriz
              </button>
            </div>
